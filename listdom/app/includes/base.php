...
class LSD_Base
{
    ...
    public function get_sf($default = [])
    {
        ...
        foreach($vars as $key=>$value)
        {
            ...
            elseif(in_array($parameter, ['s', 'shortcode']))
            {
                $sf[$parameter] = sanitize_text_field(urldecode($value));
            }
            elseif(in_array($parameter, ['adults-eq', 'children-eq']))
            {
    ...
    public static function upgradeMessage($type = 'long'): string
    {
        ...
        if($installed)
        {
            ...
            elseif($type === 'tiny') return sprintf(esc_html__("%s needed!", 'listdom'), '<a href="'.LSD_Base::getActivationURL().'"><strong>'.esc_html__('License activation', 'listdom').'</strong></a>');
    ...
    public static function minimize($number)
    {
        ...
        elseif($number < 100000) return round($number/1000, 1).'K';
        elseif($number < 1000000) return round($number/1000).'K';
